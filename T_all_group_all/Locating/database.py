...
class DatabaseConnect():
	...
	def add_loc(self,x,y,z,type):
		...
		try:
			row = cursor.execute("INSERT INTO loc (x,y,z,type) VALUES (%s,%s,%s,%s)",(x,y,z,type))
	...
	def get_all_loc(self):
		cursor = self.dbConnect.cursor()
		try:
			cursor.execute("SELECT * FROM loc order by id desc limit 30 ")
			locRows = cursor.fetchall()
			cursor.close()
			...
			for row in locRows:
				...
				locDict['z']=row[3]
				locDict['type']=row[4]
				locDict['time']=row[5].strftime("%H:%M:%S %Y/%m/%d")
